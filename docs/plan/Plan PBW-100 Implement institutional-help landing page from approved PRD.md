...
- Proposed solution:
  ...
  - Acceptance criteria checklist from PBW-100 and the confirmed interview notes:
    ...
    - [ ] The setup screen uses the full-page help entry; non-setup screens keep the current modal help behavior.
...
- Justification for simplicity:
  - Recommended option: a single `/help` route plus one small TopBar/AppHelpDialog mode branch.
  - Rejected approach 1: expand the existing `AppHelpDialog` into the full institutional page. This would create a very long modal, worsen mobile readability, complicate placeholder rendering, and directly conflict with the confirmed “keep modal on some screens” requirement.
  ...
  - The chosen path keeps the default modal behavior intact, introduces exactly one new route, uses the same token/CSS Module/component conventions already present in the app, and puts all prose where the codebase already stores user-facing copy.

## Implementation Steps

1. Lock the content and routing contract before coding the UI. Create the definitive section map from `docs/prd/institutional-page.md`, mark the public-only exclusions (`debug share preview`, `developer PWA debug tools`), decide the anchor ids, and confirm the setup screen is the only help surface moving from modal to page mode. This step should also lock the 16 placeholder ids listed above so the route, component, and locale work all target the same shape from the start.
...
6. Wire the setup-specific help entry change while preserving modal behavior everywhere else. Update `TopBar.tsx` to accept a help-entry mode prop and thread it into `AppHelpDialog.tsx`. In `AppHelpDialog.tsx`, keep the current dialog path as the default, and add a page-navigation path for setup mode that uses the same trigger button and spotlight flow but navigates to `/help` instead of opening the dialog. Then update `SetupScreen.tsx` to opt into the page mode while leaving `HistoryScreen`, `ActiveMatchScreen`, and `MatchEndScreen` unchanged. Mitigation/rollback note: keep the new branch strictly additive; if any regression appears, the immediate fallback is to switch setup back to the default dialog mode without touching the new `/help` route.
7. Add focused regression coverage and run full QA. Create `test/components/HelpLandingPage/HelpLandingPage.browser.test.tsx` to assert the page renders the expected major sections, anchor nav, localized placeholder blocks, and store CTA behavior. Update `AppHelpDialog.browser.test.tsx`, `TopBar.browser.test.tsx`, and `SetupScreen.browser.test.tsx` to cover page mode vs dialog mode, spotlight behavior, and non-regression of the existing modal path. Add or update one Playwright smoke flow to confirm setup opens the help page while a non-setup screen still opens the dialog. Finish by regenerating routes if needed, running targeted tests, and then running `pnpm complete-check`.
